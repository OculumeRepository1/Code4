...
##########################Camera Index#############################
camera_index=[0,'rtsp://admin:oculume2025@192.168.1.246:554/Preview_01_sub']
...
notification_classes = {
                    ...
                    3: 0.6,  # Class 3 threshold
                    4: 0.2,  # Class 4 threshold
                    5: 0.01,  # Class 5 threshold
                    6: 0.02,  # Class 6 threshold
                                                }
##email Rate Limiting
...
for key in cameras.keys():
    ...
    All_variables[f'last_detection_time_{key}'] = 0  # Initialize last detection time for each camera
    All_variables[f'start_detection_time_{key}'] = 0  # Initialize window active state for each camera
    
    ### Check inital image comparison folder check
    image_path = rf"C:\Users\oosma\Oculume_Codes\Dashboard\Code3\static\images\static_image_{key}.jpg"
    if not os.path.exists(image_path):
        print("Static Image does not exist. Taking picture...")
        print(f"Taking picture for camera {camera_index[key]}...")
        cap = cv2.VideoCapture(camera_index[key])
        ret, frame = cap.read()
        if ret:
        ...
        print(f"Static Image already exists at {image_path}.")
        image=cv2.imread(image_path)
        image=cv2.resize(image, (width, height))
        image=cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        All_variables[f'Static_Image_{key}'] = image
...
while True:
    try:
        ...
        database=rf"C:\Users\oosma\Oculume_Codes\Dashboard\Code3\Database\{save_dir}"
        ...
        for idx,cap in cameras.items():
            ...
            if ret:
                ...
                detection_time= All_variables[f'start_detection_time_{idx}'] - All_variables[f'last_detection_time_{idx}']
                print(f"Detection Time {detection_time}")
                if Detection or detection_time < 5:
                    if not window_active[idx]:
                        All_variables[f'start_detection_time_{idx}'] = time.time()
                    ...
                    All_variables[f'Frame_{idx}']=image
                ...
                # print(f"Tracking before update {ids} {reg_list} and idx {idx}")
                    ...
                    frame=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)                          
                    All_variables[f'Frame_{idx}']=frame
                    All_variables[F'last_detection_time_{idx}'] = time.time()
                ...
                if window_active[idx]:
                    ...
                    top=np.hstack((All_variables[f'Frame_0'], All_variables[f'Frame_0'])) 
                    bottom=np.hstack((All_variables[f'Frame_0'], All_variables[f'Frame_0'])) 
